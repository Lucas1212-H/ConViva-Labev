...
class ImportCsvOcorrencias extends Command
{
    ...
    protected $description = 'Importa as ocorrências dos arquivos CSV (2023 e 2024 via Cloudinary)';

    public function handle()
    {
        $files = $this->argument('files');
        
        // Se nenhum arquivo local for passado, busca as URLs salvas no .env
        if (empty($files)) {
            $files = array_filter([
                env('CSV_URL_2021'),
                env('CSV_URL_2022'),
        ...
        if (empty($files)) {
            $this->error("Nenhuma URL ou arquivo de CSV configurado.");
            return;
        }
    ...
    {
        // Removido o file_exists() para permitir URLs remotas (Cloudinary)
        $handle = @fopen($filePath, "r");
        
        if ($handle === false) {
            $this->error("Não foi possível abrir o arquivo/URL: $filePath");
            return;
        }
        ...
        fclose($handle);
        $this->info(" -> Foram importados/atualizados $importedCount registros.");
    }
}